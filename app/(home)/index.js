import { Alert, StyleSheet, Text, ScrollView, Pressable, TextInput, View } from 'react-native';
import React, { useState, useEffect } from 'react';
// import * as Location from "expo-router";
...
const index = () => {
    ...
    const CheckIfLocationEnabled = async () => {
        let enabled = await LocationGeoCoding.hasServicesEnabledAsync();
    ...
    const GetCurrentLocation = async () => {
        let { status } = await LocationGeoCoding.requestForegroundPermissionsAsync()
        // console.log("status",status)
        if (status !== 'granted') {
            Alert.alert(
                "Permission not granted",
                "Allow the app to use current location",
                [{ text: "OK"}],
                { cancelable: true }
            )
        }

        const location = await LocationGeoCoding.getCurrentPositionAsync({accuracy:LocationGeoCoding.Accuracy.High})
        // console.log(location)
        let { coords } = await LocationGeoCoding.getCurrentPositionAsync()
        if (coords) {
    ...
    // console.log("Pay address : ",displayCurrentAddress)
    return (
        <ScrollView style={{flex:1,backgroundColor:"#f8f8f8"}}>
            ...
            <Text styles={{color:"gray",fontSize:16,marginTop:3}}>{displayCurrentAddress}</Text>
            </View>
            <View>
                <Pressable style={{backgroundColor:"#6CB$EE",width:40,height:40,borderRadius:20,justifyContent:"center",alignItems:"center"}}>
            <Text>User</Text>
                </Pressable>
            </View>
            </View>
            <View style={{flexDirection:"row",alignItems:"center",justifyContent:"space-between",borderWidth:1,borderColor:"#C0C0C0",paddingVertical:8,paddingHorizontal:10,borderRadius:11,marginTop:10,marginHorizontal:10}}>
                <TextInput placeholder='search for hotels and resturant'/>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                {
                    recommended?.map((item,index)=>(
                        <View key={index} style={{backgroundColor:"white",flexDirection:"row",margin:10,borderRadius:8}}>
                            <View>
                ...
                <Text style={{textAlign:"center",marginTop:7,letterSpacing:4,marginBottom:5,color:"gray"}}>
                    EXPLORE
                </Text>
                <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                    {
                        items.map((item,index)=>(   
                            <View key={index} style={{width:90,borderColor:"#E0E0E0",borderWidth:1,paddingVertical:5,paddingHorizontal:1,borderRadius:5,marginLeft:10,alignItems:"center",justifyContent:"center",backgroundColor:"white"}}>
                ...
                <Text style={{textAlign:'center',marginTop:7,letterSpacing:4,marginBottom:5,color:"gray"}}>ALL RESTURANT</Text>
...
// ->"/"
// The main page of the app contain user details ,search bar to search items ,recommended hotels ,explore section and list of hotels on the app